...
import utils
import train
from src.JSFSRNet_arch import JSFSRNet
...
def main():
    ...
    model = JSFSRNet(upscale=opt.scale)
    ...
    if opt.train.lower() == 'train':
        ...
        for data_test in opt.data_test:
            ...
            PSNR, SSIM, SAM, VIF, BRI, Time = train.test(opt.dir_data + 'Test/{}/HR'.format(data_test), result_path, model, opt)
            writer.add_scalar('PSNR_{}/'.format(data_test), PSNR, 0)
            writer.add_scalar('SSIM_{}/'.format(data_test), SSIM, 0)
            writer.add_scalar('SAM_{}/'.format(data_test), SAM, 0)
            writer.add_scalar('VIF_{}/'.format(data_test), VIF, 0)
            writer.add_scalar('BRI_{}/'.format(data_test), BRI, 0)
        ...
        for epoch in range(start_epoch + 1, opt.n_epochs + 1):
            ...
            for data_test in opt.data_test:
                ...
                writer.add_scalar('SAM_{}/'.format(data_test), SAM, epoch)
                writer.add_scalar('VIF_{}/'.format(data_test), VIF, epoch)
                writer.add_scalar('BRI_{}/'.format(data_test), BRI, epoch)
        ...
        for data_test in opt.data_test:
            ...
            with open(opt.model_path + '/Results/Results_{}.csv'.format(data_test), 'w', newline='') as f:
                f_csv = csv.writer(f)
                f_csv.writerow(
                    ['image_name', 'PSNR', 'SSIM','SAM', 'VIF','BRI','Time (ms)'])
